...
void main() {
  testWithProfile(
    'tasks can scheduled and processed',
    ...
    fn: () async {
      ...
      for (final instance in instances) {
        ...
        cloud.fakeTerminateInstance(instance.instanceName);
      }
  ...
  testWithProfile(
    'Limit to 5 latest major versions',
    fn: () async {
      await taskBackend.backfillTrackingState();
      await taskBackend.runOneLoopCycle();
      ...
      expect(
        ...
        containsAll(['6.0.0', '5.1.0', '4.0.0', '3.2.0', '2.0.0']),
      );
      expect(
  ...
  testWithProfile(
    'continued scan finds new packages',
    fn: () async {
      await taskBackend.backfillTrackingState();
      await taskBackend.runOneLoopCycle();
      expect(await cloud.listInstances().toList(), hasLength(0));
  ...
  testWithProfile(
    ...
    fn: () async {
      ...
      // Use token to get the upload information
      final api = createPubApiClient(authToken: v.token);
      final uploadInfo = await api.taskUploadResult(payload.package, v.version);
  ...
  testWithProfile(
    ...
    fn: () async {
      ...
      late VersionTokenPair v;
      {
        ...
        final ps = await primaryDatabase!.db.tasksAccess.lookupOrNull('neon');
        ...
        await expectApiException(
          api.taskUploadResult(payload.package, v.version),
          status: 400,
          code: 'TaskAborted',
...
Future<void> upload(
  ...
  if (400 <= res.statusCode && res.statusCode < 500) {
    fail('HTTP error, status = ${res.statusCode}, body: ${res.body}');
  }

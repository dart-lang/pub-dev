...
import 'package:pub_dev/tool/utils/emoji.dart';
import 'package:test/test.dart';

void main() {
  group('removeEmojis', () {
    test('simple text', () {
      expect(removeEmojis('abc def 0123'), 'abc def 0123');
    });

    test('removes emoji from string', () async {
      expect(removeEmojis('📁 a 📁 b 📁'), ' a  b ');
    });

    test('keeps CJK characters', () async {
      final text = '文字 Chinese (Hanzi), 漢字 Japanese (Kanji), 漢字 Korean (Hanja)';
      expect(removeEmojis(text), text);
    });
  });
    ...
    () {
      ...
      test('basic values', () {
        ...
        for (final v in sequences.basicValues) {
          expect(removeEmojis(String.fromCharCode(v)), '',

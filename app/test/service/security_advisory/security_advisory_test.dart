...
void main() {
    ...
    try {
      final file = File(
        path.join(
          ...
          'security_advisory',
          'testdata',
          'example_advisory.json',
        ),
      ).readAsBytesSync();
      final osv = OSV.fromJson(
        utf8JsonDecoder.convert(file) as Map<String, dynamic>,
      );
      ...
      expect(
        osv.affected!.first.databaseSpecific!['source'],
  ...
  testWithProfile(
    ...
    fn: () async {
      ...
      final adv = await securityAdvisoryBackend.ingestSecurityAdvisory(
        osv,
        syncTime,
      );
      expect(adv, isNotNull);

      final adv2 = await securityAdvisoryBackend.ingestSecurityAdvisory(
        osv2,
        syncTime,
      );
      expect(adv2, isNull);
  ...
  testWithProfile(
    'List all advisories and delete advisory',
    expectedLogMessages: [
      'SHOUT Package a not found, while ingesting advisory 123.',
  ...
  testWithProfile(
    'Insert, lookup and update advisory',
    expectedLogMessages: [
      'SHOUT Package a not found, while ingesting advisory 123.',
  ...
  testWithProfile(
    ...
    fn: () async {
      ...
      expect(
        all[0].osv!.databaseSpecific!['pub_display_url'],
        'https://github.com/advisories/$ghsaId',
      );

      expect(
        all[1].osv!.databaseSpecific!['pub_display_url'],
        'https://osv.dev/vulnerability/$cveId',
      );

      expect(
        all[2].osv!.databaseSpecific!['pub_display_url'],
        'https://osv.dev/vulnerability/$id',
      );

      expect(
        all[3].osv!.databaseSpecific!['pub_display_url'],
        'https://github.com/advisories/$ghsaId',
      );

      expect(
        all[4].osv!.databaseSpecific!['pub_display_url'],
        'https://osv.dev/vulnerability/$cveId',
      );
    },
  );

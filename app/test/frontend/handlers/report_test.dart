...
void main() {
  group('Report handlers test', () {
    ...
    testWithProfile(
      'page works with signed-in session',
      fn: () async {
    ...
    testWithProfile(
      'page with missing subject',
      fn: () async {
        await expectHtmlResponse(
          await issueGet('/report'),
          present: ['&quot;subject&quot; cannot be `null`'],
    ...
    testWithProfile(
      'page with bad subject',
      fn: () async {
        await expectHtmlResponse(
          await issueGet('/report?subject=x'),
          present: ['Invalid &quot;subject&quot; parameter.'],
          absent: ['Please describe the issue you want to report:'],
          status: 400,
        );
      },
  ...
  group('Report API test', () {
    testWithProfile(
      'unauthenticated email missing',
      fn: () async {
        await withRetryPubApiClient((client) async {
          await expectApiException(
            client.postReport(ReportForm(message: 'Problem.')),
            status: 400,
            code: 'InvalidInput',
    ...
    testWithProfile(
      'authenticated email must be absent',
      fn: () async {
        await withFakeAuthRequestContext('user@pub.dev', () async {
          ...
          await withRetryPubApiClient(
            ...
            (client) async {
              await expectApiException(
                client.postReport(
                  ReportForm(email: 'any@pub.dev', message: 'Problem.'),
                ),
                status: 400,
                code: 'InvalidInput',
        ...
        await withFakeAuthRequestContext('user@pub.dev', () async {
          ...
          await withRetryPubApiClient(
            ...
            (client) async {
              await expectApiException(
                client.postReport(
                  ReportForm(subject: 'package:oxygen', message: 'Problem.'),
                ),
                status: 400,
                code: 'InvalidInput',
    ...
    testWithProfile(
      'unauthenticated report success',
      fn: () async {
        await withRetryPubApiClient((client) async {
          final msg = await client.postReport(
            ReportForm(
              email: 'user2@pub.dev',
  ...
  group('Appeal API test', () {
    Future<void> _prepareApplied({String? logSubject, String? status}) async {
    ...
    testWithProfile(
      'failure: subject is not on the case',
      fn: () async {
    ...
    testWithProfile(
      'unauthenticated appeal success, second appeal fails',
      fn: () async {
        await _prepareApplied(logSubject: 'package-version:oxygen/1.2.0');
    ...
    testWithProfile(
      'authenticated appeal success',
      fn: () async {
        await _prepareApplied(logSubject: 'package-version:oxygen/1.2.0');

        await withFakeAuthRetryPubApiClient(email: 'admin@pub.dev', (
          client,
        ) async {
          final msg = await client.postReport(
            ReportForm(

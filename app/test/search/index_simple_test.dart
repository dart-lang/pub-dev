...
server.dart adds a small, prescriptive server (PicoServer) that can be configured with different handlers for HTTP requests.''',
    ...
    test('no results via email', () async {
      final PackageSearchResult result = await index
          .search(new SearchQuery.parse(query: 'email:some@other.com'));
      expect(json.decode(json.encode(result)), {
        'indexUpdated': isNotNull,
        'totalCount': 0,
        'packages': [],
    ...
    test('no results via email domain', () async {
      final PackageSearchResult result =
          await index.search(new SearchQuery.parse(query: 'email:@other.com'));
      expect(json.decode(json.encode(result)), {
        'indexUpdated': isNotNull,
        'totalCount': 0,
        'packages': [],
    ...
    test('filter by email domain', () async {
      final PackageSearchResult result = await index
          .search(new SearchQuery.parse(query: 'email:@example.com'));
      expect(json.decode(json.encode(result)), {
        'indexUpdated': isNotNull,
        'totalCount': 2,
        'packages': [

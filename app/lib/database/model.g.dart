...
extension PrimaryDatabaseSchema on Database<PrimaryDatabase> {
  ...
  Table<Task> get tasks => ExposedForCodeGen.declareTable(
        this,
        _$Task._$table,
      );

  Table<TaskDependency> get taskDependencies => ExposedForCodeGen.declareTable(
        this,
...
final class _$Task extends Task {
  ...
  static Task? _$fromDatabase(RowReader row) {
    ...
    return _$Task._(runtimeVersion!, package!, state!, pendingAt!,
        lastDependencyChanged!, finished!);
  }
...
extension QueryTaskExt on Query<(Expr<Task>,)> {
  ...
  QuerySingle<(Expr<Task>,)> byKey(
    String runtimeVersion,
    String package,
  ) =>
      where((task) =>
          task.runtimeVersion.equalsValue(runtimeVersion) &
          task.package.equalsValue(package)).first;
...
extension ExpressionTaskExt on Expr<Task> {
  ...
  SubQuery<(Expr<TaskDependency>,)> get dependencies =>
      ExposedForCodeGen.subqueryTable(_$TaskDependency._$table).where((r) =>
...
extension InnerJoinTaskTaskDependencyExt
  ...
  Query<(Expr<Task>, Expr<TaskDependency>)> usingTask() => on((a, b) =>
...
extension InnerJoinTaskDependencyTaskExt
  ...
  Query<(Expr<TaskDependency>, Expr<Task>)> usingTask() => on((a, b) =>
...
extension TaskStateExt on TaskState {
  ...
  /// Wrap this [TaskState] as [Expr<TaskState>] for use queries with
  /// `package:typed_sql`.
  Expr<TaskState> get asExpr => ExposedForCodeGen.literalCustomDataType(
        this,
        _exprType,
      ).asNotNull();
}

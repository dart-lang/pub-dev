...
abstract class ImportSource {
  ...
  ///
  /// [referenceTime] is used to make time-based operations e.g. version created
  /// timestamp consistent.
  static ImportSource autoGenerated({
    DateTime referenceTime,
  }) {
    return _AutoGeneratedImportSource(referenceTime: referenceTime);
  }
}
...
class _PubDevImportSource implements ImportSource {
  ...
  Future<List<ResolvedVersion>> resolveVersions(TestProfile profile) async {
    ...
    // resolve all the required versions with the pub tool
    final resolved = await resolver.resolveVersions(profile);

    // get published date from pub.dev
    for (final v in resolved) {
      final rs = await _client.get(
          '${urls.siteRoot}/api/packages/${v.package}/versions/${v.version}');
      final map = json.decode(rs.body) as Map<String, dynamic>;
      final info = package_api.VersionInfo.fromJson(map);
      versions.add(v.change(created: info.published));
    }
    return versions;
  }
...
class _AutoGeneratedImportSource implements ImportSource {
  ...
  _AutoGeneratedImportSource({DateTime referenceTime})
      : _referenceTime = referenceTime ?? DateTime.now().toUtc();
  ...
  Future<List<ResolvedVersion>> resolveVersions(TestProfile profile) async {
    ...
    profile.packages.forEach((p) {
      ...
      var lastCreated = _referenceTime;

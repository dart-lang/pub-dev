...
Future<(CreateInstancesState, Duration)> runOneCreateInstancesCycle(
  ...
  Future<void> scheduleInstance(({String package}) selected) async {
    ...
    if (rollbackPackageState) {
      ...
      await db.tasksAccess.restorePreviousVersionsState(
        selected.package,
        instanceName,
        oldVersionsMap,
      );
  ...
  await Future.wait(
    (await db.tasksAccess.selectSomePending(selectLimit).toList()).map(
      scheduleInstance,
    ),
  );
...
updatePackageStateWithPendingVersions(
  ...
  return schema.transactWithRetry(() async {
    ...
    final oldVersionsMap = {...task.state.versions};

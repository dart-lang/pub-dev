...
FROM google/dart-runtime-base:2.1.0-dev.5.0
...
RUN cd / && \
    curl -sS https://storage.googleapis.com/dart-archive/channels/stable/release/2.0.0/sdk/dartsdk-linux-x64-release.zip >/dartsdk.zip && \
    unzip -q /dartsdk.zip && \
    rm -f /dartsdk.zip

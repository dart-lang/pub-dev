...
FROM google/dart-runtime-base:2.5.0
...
RUN cd / && \
  curl -sS https://storage.googleapis.com/dart-archive/channels/stable/raw/2.6.0/sdk/dartsdk-linux-x64-release.zip >/dartsdk.zip && \
  unzip -q /dartsdk.zip && \
  rm -f /dartsdk.zip

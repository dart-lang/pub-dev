...
/*
code.Code _buildClientClass({
  @required ClassElement classElement,
  @required List<_Handler> handlers,
}) =>
    Code('''
/// Client for calling [${classElement.name}] through generated [Router].
class ${classElement.name}Client {
  final String _baseUrl;
  final http.Client _client;

  ${classElement.name}Client(this._baseUrl, [this._client]);

  ${handlers.map(_buildClientMethod).join('\n')}
}
''');

final _parser = RegExp(r'([^<]*)(?:<([^>|]+)(?:\|([^>]*))?>)?');

String _buildClientMethod(_Handler handler) {
  final name = handler.element.displayName;
  ...
  final params = [
    for (final p in routeParams) 'String $p',
    if (hasBody) '$bodyType body',
  ];

  final urlPattern = '\$baseUrl' +
      handler.route.replaceAllMapped(
        _parser,
        (m) => m[2] != null ? '${m[1]}\$${m[2]}' : m[1],
      );
  var getBody = '';
  ...
  return '''
  Future<http.Response> $name(${params.join(',')}) async {
    final res = sendRequest(_client, '${handler.verb}', '$urlPattern', $getBody);

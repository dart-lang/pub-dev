  ...
  if [[ ${EXIT_CODE} -ne 0 ]]; then
    ...
    for TASK in "$@"; do
      ...
      test_08)
        ...
        ;;
      test_09)
        echo 'dart test -j1 --run-skipped `find test -name "*_test\\.dart" | sort | sed -n '0~3p'`'
        dart test -j1 --run-skipped `find test -name "*_test\\.dart" | sort | sed -n '0~3p'` || EXIT_CODE=$?
        ;;
      test_10)
        echo 'dart test -j1 --run-skipped `find test -name "*_test\\.dart" | sort | sed -n '1~3p'`'
        dart test -j1 --run-skipped `find test -name "*_test\\.dart" | sort | sed -n '1~3p'` || EXIT_CODE=$?
        ;;
      test_11)
        echo 'dart test -j1 --run-skipped `find test -name "*_test\\.dart" | sort | sed -n '2~3p'`'
        dart test -j1 --run-skipped `find test -name "*_test\\.dart" | sort | sed -n '2~3p'` || EXIT_CODE=$?
        ;;
      test_12)
